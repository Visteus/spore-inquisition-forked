...
execute as @e[tag=ordeal_runner,limit=1] at @p if score !finale proto >= !finale ord_thr1 run summon spore:vigil ~ ~-10 ~ {PersistenceRequired:1b}
execute as @e[tag=ordeal_runner,limit=1] at @p if score !finale proto >= !finale ord_thr2 run summon spore:vigil ~ ~-10 ~ {PersistenceRequired:1b}
execute as @e[tag=ordeal_runner,limit=1] at @p if score !finale proto >= !finale ord_thr3 run summon spore:vigil ~ ~-10 ~ {PersistenceRequired:1b}
execute as @e[tag=ordeal_runner,limit=1] at @p if score !finale proto >= !finale ord_thr4 run summon spore:howit_arm ~ ~-10 ~ {PersistenceRequired:1b}
